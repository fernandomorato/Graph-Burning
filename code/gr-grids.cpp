...
void parse_args(int argc, char **argv, long long &seed, double &alpha, int &time_limit, int &criterio, string &input_path, string &output_path, string &log_path, string &alpha_path) {
	bool f_seed  = false;
...
int main(int argc, char **argv) {
	...
	string log_path;
	string alpha_path;
	parse_args(argc, argv, seed, alpha, time_limit, criterio, input_path, output_path, log_path, alpha_path);
	mt19937 rng(seed);
	// File Variables
	...
	do {
		n_iterations++;
		pair<vector<int>, bool> ans = construction(n_iterations, centrality_scores, n_vertices, n_edges, graph_density, rng, alpha, criterio, 6);
		if (!ans.second) {
